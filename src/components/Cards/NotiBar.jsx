...
export function PopoverDemo() {
  ...
  return (
   <>
    {noti.length >0 && <div className="absolute -top-2 -right-3.5 flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full dark:border-gray-900">
                  {noti.length}
                </div>}
    <Popover>
          ...
          <div className="space-y-2">
            ...
            <div className="">
              {noti.reverse().map((noti) => {
                return (
                  <div key={noti.id} className="flex justify-between py-2 cursor-pointer hover:bg-gray-50 p-1 rounded pe-2">
                    <p className="text-sm text-muted-foreground flex items-center space-x-2">
                      <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                      <span>{noti.message}</span>
                    </p>
                    <p className="text-xs text-muted-foreground">{noti.type}</p>
                  </div>
                );
              })}
            </div>
