...
import { ServiceCenterDetailsModal } from "../../../components/warehouse/service-centers/service-center-details-modal"
...
export default function Orders() {
  ...
  const [isAssigningServiceCenter, setIsAssigningServiceCenter] = useState(false)
  ...
  const handleAssignTruck = (orderIds, truckId) => {
    setOrders(orders.map((order) => (orderIds.includes(order.id) ? { ...order, assignedTruck: truckId } : order)))
    setSelectedOrders([])
    setIsAssigningTruck(false)
    toast({
      title: "Orders Assigned to Truck",
      description: `Selected orders have been assigned to truck ${truckId}`,
    })
  }

  const handleAssignServiceCenter = (orderIds, serviceCenter) => {
    setOrders(orders.map((order) => (orderIds.includes(order.id) ? { ...order, serviceCenter: serviceCenter } : order)))
    setSelectedOrders([])
    setIsAssigningServiceCenter(false)
    toast({
      title: "Orders Assigned to Service Center",
      description: `Selected orders have been assigned to ${serviceCenter}`,
    })
  }
    ...
    <div className="space-y-6">
      <Card className="bg-white shadow-lg">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          {selectedOrders.length > 0 && (
            <div className="flex space-x-2">
              <Button onClick={() => setIsAssigningTruck(true)} className="flex items-center">
                <Truck className="mr-2 h-4 w-4" />
                Assign Trucks
              </Button>
              <Button onClick={() => setIsAssigningServiceCenter(true)} className="flex items-center">
        ...
        <CardContent>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            />
            {selectedOrder && <OrderDetails order={selectedOrder} onStatusUpdate={handleStatusUpdate} />}
          </div>
        </CardContent>
      </Card>
      ...
      {isAssigningServiceCenter && (
        <ServiceCenterAssignmentModal
          orders={orders.filter((order) => selectedOrders.includes(order.id))}
          onAssign={(serviceCenter) => handleAssignServiceCenter(selectedOrders, serviceCenter)}
